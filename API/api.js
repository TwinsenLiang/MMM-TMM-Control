...
module.exports = {
   ...
   */
  getApiKey () {
    const thisConfig = this.configOnHd.modules.find((x) => x.module === "MMM-TMM-Control");
  ...
  getExternalApiByGuessing () {
    ...
    const skippedModules = ["clock", "compliments", "MMM-TMM-Control"];
  ...
  createApiRoutes () {
    ...
    this.expressRouter.route("/classes/:value").
      get((req, res) => {
        const classes = self.getConfig().modules.find((m) => m.module === "MMM-TMM-Control").config || {};
    ...
    this.expressRouter.route(route).
      get((req, res) => {
        ...
        switch (req.params.moduleName) {
          ...
          case "rc": case "RC": this.updateModule("MMM-TMM-Control", res); break;
